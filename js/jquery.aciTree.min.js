...
(function(i,j,h){var f={container:"ul",item:"li",distance:4,handle:"*",disable:"a,input,textarea,select,option,button",child:null,childHolder:'<ul class="aciSortableChild"></ul>',childHolderSelector:".aciSortableChild",exclude:null,vertical:true,placeholder:'<li class="aciSortablePlaceholder"><div></div></li>',placeholderSelector:".aciSortablePlaceholder",helper:'<div class="aciSortableHelper"></div>',helperSelector:".aciSortableHelper",relative:false,draggable:true,gluedPlaceholder:false,connectDrop:null,dropPosition:null,simpleDrop:null,scroll:80,scrollParent:"window",before:function(a){if(this._instance.options.exclude){var b=this.containerFrom(a);return !b.is(this._instance.options.exclude)&&!a.is(this._instance.options.exclude)}return true},start:function(c,b,d){var a=c.clone();a.children(this._instance.options.container).remove();d.html(a.text())},valid:function(c,e,b,m,a,d){if(this._instance.options.exclude){if(m){return !e.is(this._instance.options.exclude)}else{var n=this.containerFrom(e);return !n.is(this._instance.options.exclude)}}return true},drag:function(c,a,b,d){},create:function(a,b){b.append(this._instance.options.childHolder);return true},remove:function(a,b){b.children(this._instance.options.childHolderSelector).remove()},end:function(b,d,a,c){if(a.parent().length){a.after(b).detach()}c.detach()}};var g={__extend:function(){i.extend(this._instance,{sorting:false,item:null,hoverItem:null,isContainer:false,pointStart:null,pointNow:null,placeholder:null,helper:null,relative:null,children:null,scroll:false,lastCheck:{}})},init:function(){if(this.wasInit()){return}this._instance.jQuery.on("mousedown"+this._instance.nameSpace,this._instance.options.item,this.proxy(function(b){var a=i(b.target);if(!a.is(this._instance.options.handle)||a.is(this._instance.options.disable)){return}if(!a.is(this._instance.options.disable)){b.preventDefault()}if(a.is(this._instance.options.container)){i(j.document.body).css("cursor","no-drop")}else{this._delayStart(b)}})).on("mousemove"+this._instance.nameSpace,this._instance.options.item,this.proxy(function(a){if(this._instance.sorting){a.stopPropagation();this._instance.isContainer=false;var b=this.itemFrom(a.target);if(this._instance.item.has(b).length){this._instance.hoverItem=null}else{if(this._instance.options.dropPosition===null){this._instance.hoverItem=b}else{var c=this.containerFrom(a.target);this._dropPosition(c)}}}this._drag(a)})).on("mousemove"+this._instance.nameSpace,this._instance.options.container,this.proxy(function(a){if(this._instance.sorting){a.stopPropagation();var b=this.containerFrom(a.target);if(!this._instance.item.has(b).length){if(this.isEmpty(b)){this._instance.hoverItem=b;this._instance.isContainer=true}else{this._instance.isContainer=false;if(this._instance.options.dropPosition===null){this._instance.hoverItem=this._closestFrom(a)}else{this._dropPosition(b)}}}}this._drag(a)}));this._initConnect();this._initSimple();i(j.document).bind("mousemove"+this._instance.nameSpace+this._instance.index,this.proxy(function(a){if(this._instance.sorting){this._instance.hoverItem=null;this._drag(a)}})).on("mousemove"+this._instance.nameSpace+this._instance.index,this._instance.options.helperSelector,this.proxy(function(a){if(this._instance.sorting){var b=this._fromCursor(a);if(b){this._instance.jQuery.trigger(i.Event("mousemove",{target:b,pageX:a.pageX,pageY:a.pageY}));a.stopPropagation()}}})).bind("selectstart"+this._instance.nameSpace+this._instance.index,this.proxy(function(a){if(this._instance.sorting){a.preventDefault()}})).bind("mouseup"+this._instance.nameSpace+this._instance.index,this.proxy(function(){if(this._instance.sorting){this._end()}else{this._instance.item=null;i(j.document.body).css("cursor","default")}}));this._super()},_dropPosition:function(a){if(this._instance.options.dropPosition==-1){this._instance.hoverItem=this._firstItem(a)}else{this._instance.hoverItem=this._lastItem(a)}if(!this._instance.hoverItem.length){this._instance.hoverItem=a;this._instance.isContainer=true}},_firstItem:function(a){return a.children(this._instance.options.item).not(this._instance.options.placeholderSelector).first()},_lastItem:function(a){return a.children(this._instance.options.item).not(this._instance.options.placeholderSelector).last()},_closestFrom:function(a){var b=null;var c=i(a.target);if(this._instance.options.vertical){var e=i(j).scrollTop();var d=c.height();c.find(this._instance.options.item).not(this._instance.options.placeholderSelector).each(function(){var n=this.getBoundingClientRect();var m=j.Math.abs(e+n.top+(n.bottom-n.top)/2-a.pageY);if(m<d){d=m;b=i(this)}})}else{var e=i(j).scrollLeft();var d=c.width();c.find(this._instance.options.item).not(this._instance.options.placeholderSelector).each(function(){var n=this.getBoundingClientRect();var m=j.Math.abs(e+n.left+(n.right-n.left)/2-a.pageX);if(m<d){d=m;b=i(this)}})}return b},_fromCursor:function(a){if(this._instance.helper&&this._instance.helper.parent().length){if(i(a.target).closest(this._instance.options.helperSelector).length){this._instance.helper.hide();var b=j.document.elementFromPoint(a.clientX,a.clientY);this._instance.helper.show();if(b&&(b!=j.document.body)){return b}}}return null},_initConnect:function(){if(this._instance.options.connectDrop){i(j.document).on("mousemove"+this._instance.nameSpace+"connect"+this._instance.index,this._instance.options.connectDrop,this.proxy(function(a){var b=i(a.target);if(this._instance.sorting&&!this._instance.jQuery.has(b).length){if(!b.is(this._instance.options.connectDrop)){b=b.closest(this._instance.options.connectDrop)}this._connect(b)}}))}},_doneConnect:function(){i(j.document).off(this._instance.nameSpace+"connect"+this._instance.index)},_initSimple:function(){if(this._instance.options.simpleDrop){i(j.document).on("mousemove"+this._instance.nameSpace+"simple"+this._instance.index,this._instance.options.simpleDrop,this.proxy(function(a){var b=i(a.target);if(this._instance.sorting&&!this._instance.jQuery.has(b).length){a.stopPropagation();if(!b.is(this._instance.options.simpleDrop)){b=b.closest(this._instance.options.simpleDrop)}this._instance.hoverItem=b;this._instance.isContainer=false}this._drag(a)}))}},_doneSimple:function(){i(j.document).off(this._instance.nameSpace+"simple"+this._instance.index)},_trigger:function(c,a){var b=i.Event("acisortable");this._instance.jQuery.trigger(b,[this,c,a]);return !b.isDefaultPrevented()},_call:function(b,a){var c=[];for(var d in a){c.push(a[d])}var e=true;if(!this._trigger("before"+b,a)){return false}if(this._instance.options[b]){e=this._instance.options[b].apply(this,c)}if(!this._trigger(b,a)){e=false}return e},_connect:function(c){var a=c.aciSortable("api");if(a.wasInit()){a._instance.item=this._instance.item;a._instance.pointStart=this._instance.pointStart;a._instance.relative=this._instance.relative;a._instance.helper=this._instance.helper.clone();var b=i('<div style="display:none"></div>');i(j.document.body).append(b);this._instance.item=b;this._instance.placeholder.detach();this._instance.helper.detach();this._end();b.remove();a._start()}},hasItem:function(a){return this._instance.jQuery.has(a).length>0},sortableFrom:function(a){if(this._instance.jQuery.has(a).length){return this._instance.jQuery}if(this._instance.options.connectDrop){return i(a).closest(this._instance.options.connectDrop)}return i()},itemFrom:function(a){return i(a).closest(this._instance.options.item)},containerFrom:function(a){return i(a).closest(this._instance.options.container)},hasChildrens:function(a){return a.children(this._instance.options.container).children(this._instance.options.item).not(this._instance.options.placeholderSelector).length>0},hasContainer:function(a){return a.children(this._instance.options.container).length>0},isEmpty:function(a){return !a.children(this._instance.options.item).not(this._instance.options.placeholderSelector).length},_delayStart:function(c){var d=this.itemFrom(c.target);if(this._call("before",{item:d})){this._instance.item=d;this._instance.pointStart={x:c.pageX,y:c.pageY};if(this._instance.options.relative){var a=i(j).scrollTop();var b=i(j).scrollLeft();var e=this._instance.item.get(0).getBoundingClientRect();this._instance.relative={x:e.left+b-c.pageX,y:e.top+a-c.pageY}}else{this._instance.relative={x:0,y:0}}this._drag(c)}else{i(j.document.body).css("cursor","no-drop")}},_start:function(){this._instance.sorting=true;if(!this._instance.placeholder){this._instance.placeholder=i(this._instance.options.placeholder)}if(!this._instance.helper){this._instance.helper=i(this._instance.options.helper)}this._call("start",{item:this._instance.item,placeholder:this._instance.placeholder,helper:this._instance.helper});if(this._instance.options.gluedPlaceholder){this._instance.item.after(this._instance.placeholder)}i(j.document.body).append(this._instance.helper);i(j.document.body).css("cursor","move")},_minDistance:function(){return(j.Math.abs(this._instance.pointStart.x-this._instance.pointNow.x)>this._instance.options.distance)||(j.Math.abs(this._instance.pointStart.y-this._instance.pointNow.y)>this._instance.options.distance)},_drag:function(a){this._instance.pointNow={x:a.pageX,y:a.pageY};if(this._instance.sorting){this._helper();this._placeholder()}else{if(this._instance.item){if(this._minDistance()){this._start()}}}},_onDrag:function(a){this._call("drag",{item:this._instance.item,placeholder:this._instance.placeholder,isValid:a,helper:this._instance.helper})},_isValid:function(a,b){if(!this._instance.options.draggable&&this.hasItem(this._instance.item)&&(this._instance.isContainer||(this.containerFrom(this._instance.item).get(0)!=this.containerFrom(this._instance.hoverItem).get(0)))){return false}if(this._call("valid",{item:this._instance.item,hover:this._instance.hoverItem,before:b?null:((a===null)?false:a),isContainer:b?false:this._instance.isContainer,placeholder:this._instance.placeholder,helper:this._instance.helper})){i(j.document.body).css("cursor","move");return true}return false},_onCreate:function(){this._onRemove();if(this._instance.hoverItem.is(this._instance.options.exclude)){return false}if(this._isSimple()){return false}if(this._call("create",{item:this._instance.item,hover:this._instance.hoverItem})){this._instance.childItem=this._instance.hoverItem;this._instance.hoverItem=this._instance.hoverItem.children(this._instance.options.childHolderSelector);this._instance.isContainer=true;this._placeholder();return true}return false},_onRemove:function(a){if(this._instance.childItem&&(!a||(a.get(0)!=this._instance.childItem.get(0)))){if(this._instance.placeholder){this._instance.placeholder.detach()}if(!this.hasChildrens(this._instance.childItem)){this._call("remove",{item:this._instance.item,hover:this._instance.childItem})}this._instance.childItem=null}},_wasValid:function(a,c,d){if(this._instance.lastCheck){var b=d?this._instance.lastCheck.check:this._instance.lastCheck.normal;var e=b&&(b.hoverItem.get(0)==this._instance.hoverItem.get(0))&&(b.before===a)&&(b.create===c)&&(b.isContainer==this._instance.isContainer)}this._instance.lastCheck[d?"check":"normal"]={hoverItem:this._instance.hoverItem,before:a,create:c,isContainer:this._instance.isContainer};return e},_isSimple:function(){return this._instance.options.simpleDrop&&this._instance.hoverItem&&this._instance.hoverItem.is(this._instance.options.simpleDrop)},_placeholder:function(){this._instance.pointStart=this._instance.pointNow;if(this._instance.hoverItem){if(this._instance.hoverItem.is(this._instance.options.placeholderSelector)){return}if(this._instance.isContainer){if(this._wasValid(null,false)){return}if(this._isValid(null)){this._instance.hoverItem.append(this._instance.placeholder);this._onDrag(true);return}}else{if(this._instance.hoverItem.get(0)!=this._instance.item.get(0)){var c=false,p=false;var e=this._instance.hoverItem.get(0).getBoundingClientRect();var q=this._instance.hoverItem.children(this._instance.options.container);if(this._instance.options.vertical){var b=i(j).scrollTop();var s=e.bottom-((q.length&&q.is(":visible"))?q.outerHeight(true):0);if(this._instance.options.child&&(this._instance.options.draggable||!this.hasItem(this._instance.item))&&!this.hasChildrens(this._instance.hoverItem)){var r=(s-e.top)*(0.5-this._instance.options.child/200);if((this._instance.pointNow.y>b+e.top+r)&&(this._instance.pointNow.y<b+s-r)){p=q.length?null:true}}if(this._instance.pointNow.y<b+e.top+(s-e.top)/2){c=true}}else{var b=i(j).scrollLeft();var a=e.right-((q.length&&q.is(":visible"))?q.outerWidth(true):0);if(this._instance.options.child&&(this._instance.options.draggable||!this.hasItem(this._instance.item))&&!this.hasChildrens(this._instance.hoverItem)){var r=(a-e.left)*(0.5-this._instance.options.child/200);if((this._instance.pointNow.x>b+e.left+r)||(this._instance.pointNow.x<b+a-r)){p=q.length?null:true}}if(this._instance.pointNow.x<b+e.left+(a-e.left)/2){c=true}}if(this._instance.options.dropPosition==-1){c=true}else{if(this._instance.options.dropPosition==1){c=false}}if(p!==false){if(this._wasValid(c,p,true)){return}if(this._isValid(null,true)===false){p=false}}if(p===null){this._onRemove(this._instance.hoverItem);this._instance.childItem=this._instance.hoverItem;this._instance.hoverItem=this._instance.hoverItem.children(this._instance.options.childHolderSelector);this._instance.isContainer=true;if(this._wasValid(null,true)){return}if(this._isValid(null)){if(!this._isSimple()){this._instance.hoverItem.append(this._instance.placeholder)}this._onDrag(true);return}}else{if(c){if(this._wasValid(true,p)){return}if(this._isValid(true)){if(p&&this._onCreate()){return}var d=this._instance.hoverItem.prev(this._instance.options.item);if(this._instance.options.gluedPlaceholder||(d.get(0)!=this._instance.item.get(0))){if(!this._isSimple()){this._instance.hoverItem.before(this._instance.placeholder)}this._onDrag(true);return}}}else{if(this._wasValid(false,p)){return}if(this._isValid(false)){if(p&&this._onCreate()){return}var t=this._instance.hoverItem.next(this._instance.options.item);if(this._instance.options.gluedPlaceholder||(t.get(0)!=this._instance.item.get(0))){if(!this._isSimple()){this._instance.hoverItem.after(this._instance.placeholder)}this._onDrag(true);return}}}}}}}else{this._instance.lastCheck={}}if(!this._instance.options.gluedPlaceholder){this._instance.placeholder.detach()}i(j.document.body).css("cursor","no-drop");this._onDrag(false)},_scrollParent:function(a){if(a){a=a.parents(this._instance.options.scrollParent).first()}else{var a=this._instance.jQuery.parents(this._instance.options.scrollParent).first()}if(a.length){if(!this._scrollContainer(a)){return a}}else{if(this._instance.options.scrollParent.match(/^(.*,)?window(,.*)?$/)){this._scrollContainer(i(j),true)}}return null},_amount:function(b,a){return b/(1+j.Math.pow(1.04,-a+60))},_scrollContainer:function(y,v){var e=false;var c=i(j).scrollTop(),u=i(j).scrollLeft(),b=y.height(),w=y.width();var a=v?j.document.body.scrollHeight:y.get(0).scrollHeight;var t=j.Math.min(this._instance.options.scroll,b/3);var d=v?{left:0,top:0,right:w,bottom:b}:y.get(0).getBoundingClientRect();if((a>b)&&(this._instance.pointNow.x>u+d.left)&&(this._instance.pointNow.x<u+d.right)){var x=y.scrollTop();if(this._instance.pointNow.y<c+d.top+t){if(x>0){var z=c+d.top+t-this._instance.pointNow.y;y.scrollTop(j.Math.max(x-this._amount(t,z),0));e=true}}else{if(this._instance.pointNow.y>c+d.bottom-t){if(x+b<a){var z=this._instance.pointNow.y-(c+d.bottom-t);y.scrollTop(j.Math.min(x+this._amount(t,z),a-b));e=true}}}}var s=v?j.document.body.scrollWidth:y.get(0).scrollWidth;t=j.Math.min(this._instance.options.scroll,w/3);if((s>w)&&(this._instance.pointNow.y>c+d.top)&&(this._instance.pointNow.y<c+d.bottom)){var x=y.scrollLeft();if(this._instance.pointNow.x<u+d.left+t){if(x>0){var z=u+d.left+t-this._instance.pointNow.x;y.scrollLeft(j.Math.max(x-this._amount(t,z),0));e=true}}else{if(this._instance.pointNow.x>u+d.right-t){if(x+w<s){var z=this._instance.pointNow.x-(u+d.right-t);y.scrollLeft(j.Math.min(x+this._amount(t,z),s-w));e=true}}}}if(v&&e){this._instance.pointNow.x+=y.scrollLeft()-u;this._instance.pointNow.y+=y.scrollTop()-c}return e},_scroll:function(){if(this._instance.scroll){return}this._instance.scroll=true;if(!this._scrollContainer(this._instance.jQuery)&&this._instance.options.scrollParent){var a=null;while(true){a=this._scrollParent(a);if(a===null){break}}}if(this._instance.sorting){j.setTimeout(this.proxy(function(){this._instance.scroll=false;this._helper()}),50)}else{this._instance.scroll=false}},_helper:function(){if(this._instance.options.scroll){this._scroll()}this._instance.helper.css({left:(this._instance.pointNow.x+this._instance.relative.x)+"px",top:(this._instance.pointNow.y+this._instance.relative.y)+"px"})},_end:function(){if(this._instance.placeholder.parent().length){if((this._instance.placeholder.prev(this._instance.options.item).get(0)==this._instance.item.get(0))||(this._instance.placeholder.next(this._instance.options.item).get(0)==this._instance.item.get(0))){this._instance.placeholder.detach()}}var a=this.containerFrom(this._instance.item);this._call("end",{item:this._instance.item,hover:this._instance.hoverItem,placeholder:this._instance.placeholder,helper:this._instance.helper});this._onRemove();var b=this.itemFrom(a);if(b.length&&!this.hasChildrens(b)){this._instance.childItem=b;this._onRemove()}this._instance.sorting=false;this._instance.item=null;this._instance.hoverItem=null;this._instance.lastCheck={};i(j.document.body).css("cursor","default")},option:function(b,a){if(this.wasInit()){switch(b){case"connectDrop":if(a!=this._instance.options.connectDrop){this._doneConnect();this._instance.options.connectDrop=a;this._initConnect()}break;case"simpleDrop":if(a!=this._instance.options.simpleDrop){this._doneSimple();this._instance.options.simpleDrop=a;this._initSimple()}break}}this._super(b,a)},destroy:function(){if(!this.wasInit()){return}this._instance.jQuery.off(this._instance.nameSpace);this._doneConnect();i(j.document).unbind(this._instance.nameSpace+this._instance.index).off(this._instance.nameSpace+this._instance.index);if(this._instance.placeholder){this._instance.placeholder.detach()}if(this._instance.helper){this._instance.helper.detach()}this._super()}};aciPluginClass.plugins.aciSortable=aciPluginClass.aciPluginUi.extend(g,"aciSortable");aciPluginClass.publish("aciSortable",f)})(jQuery,this);
